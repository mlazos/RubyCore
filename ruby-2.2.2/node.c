...
rb_parser_dump_tree(NODE *node, int comment)
{
	...
	const char *str = rb_to_core(node);
...
*rb_to_core(NODE *node) {
	...
	*/
	...
	out = SCATF(out, handle_args(node));	
	return CLOSP(out);	
					   }
      case NODE_IF: 
	...
	ANN("example; def foo; bar; end");
	*/
	out = SCATF(STR("(let "), PADR(ID2STR(node->nd_mid)));
	...
	goto asgn;
	  ...
	  case NODE_DASGN:
	/*dynamic var (ex. x = nil; 1.times{ x = foo })
	 * [nd_vid](dvar) = [nd_value]*/
	goto asgn;
